...
## Процесс разработки
- Основная рабочая ветка: `dev` (создаётся от `main`).
- Для merge-conflict в PR используется стратегия приоритета текущей ветки (`ours`) для бизнес-критичных файлов конфигурации/интеграций.
- Сообщения коммитов в репозитории ведутся на русском языке.
